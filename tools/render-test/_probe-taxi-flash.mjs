...
const out = await page.evaluate(() => {
  ...
  return {
    ...
    playback: p.playback,
    renderTileCounts: p.renderLayers.map((r) => ({ l: r.layer, t: r.tiles, s: r.sublayers })),
...
for (const b of out.backjumps.slice(0, 20)) {
  console.log(`  wall=${b.wall.toFixed(0)} deltaSimMs=${b.deltaSimMs.toFixed(0)}`);
}
...
console.log(`timeline samples: ${out.timeline.length}, small regressions: ${regressions}`);
...
const removalsOnly = out.tileSwaps.filter((s) => s.removed.length > 0 && s.added.length === 0);
...
for (const s of out.tileSwaps) {
  ...
  for (const k of s.added) {
    if (removedAt.has(k) && s.wall - removedAt.get(k) < 5000) {
      readds++;
      if (readds <= 10) console.log(`  re-add ${k} after ${(s.wall - removedAt.get(k)).toFixed(0)}ms`);
    }
  }
}
...
for (const e of out.playback.slice(0, 30)) {
  console.log(`  ${e.event} state=${e.state} stalls=${e.stallCount ?? '?'} creepMs=${e.creepMs?.toFixed?.(0) ?? '?'}`);
}
...
console.log(`renderLayers samples: ${counts.length}, transient tile-count dips: ${dips}`);
...
console.log(`batches: priority=${prio.length} (tiles ${prio.reduce((a,b)=>a+b.n,0)}), prefetch=${pref.length} (tiles ${pref.reduce((a,b)=>a+b.n,0)})`);
