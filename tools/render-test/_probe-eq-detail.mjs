...
// Click play
const playBtn = page.locator('button[aria-label="Play"], button:has-text("▶")').first();
if (await playBtn.count()) {
  await playBtn.click();
  await page.waitForTimeout(4000);
  await page.screenshot({ path: `${OUT}/detail-02-playing.png` });
  // Stop so we capture a stable frame
  const pauseBtn = page.locator('button[aria-label="Pause"], button:has-text("⏸")').first();
...
if (viewport) {
  await page.screenshot({
    ...
    clip: { x: viewport.x, y: viewport.y, width: viewport.width, height: viewport.height - 80 },
  });
}
