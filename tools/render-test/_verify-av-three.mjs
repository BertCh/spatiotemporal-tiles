...
const SCENES = (process.env.SCENE || 'argoverse-0bae3b5e,argoverse-0bae3b5e-surfel')
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean);
...
async function verifyScene(scene) {
  ...
  // Switch deck → TSL.
  await page.evaluate(() => {
    const b = Array.from(document.querySelectorAll('button')).find((x) => /deck\.gl|TSL/.test(x.textContent || ''));
  ...
  const buildThrows = errors.filter((e) => /reading 'build'|not found on geometry/.test(e));
  ...
  console.log(`RESULT(${scene}): noBuildThrows=${buildThrows.length === 0} noZeroDraws=${zeroDraws.length === 0}\n`);
  await ctx.close();
  return clean;
}
...
console.log(`VERIFY (${SCENES.length} scene${SCENES.length === 1 ? '' : 's'}): ${allClean ? 'PASS' : 'FAIL'}`);
await browser.close();
