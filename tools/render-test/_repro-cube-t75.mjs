...
async function bounds(page) {
  return page.locator('input[type="range"]').first().evaluate((el) => ({
...
async function run(label, steps) {
  ...
  await page.goto(`${BASE_URL}/demo/nyc-taxi-cube`, { waitUntil: 'domcontentloaded', timeout: 60_000 });
  ...
  const probe = await page.evaluate(() => {
    ...
    if (!p) return null;
    try { return JSON.parse(JSON.stringify(p.snapshot?.() ?? p)); } catch { return String(p); }
  }).catch(() => null);
  console.log(`=== ${label} ===`);
