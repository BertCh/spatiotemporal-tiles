...
await page.goto(`${BASE}/demo/ship-traffic`, { waitUntil: 'domcontentloaded', timeout: 60_000 });
...
for (let i = 0; i < 4; i++) { await page.mouse.wheel(0, -240); await page.waitForTimeout(450); }
...
const census = await page.evaluate(() => {
  ...
  const top = [...counts.entries()].sort((a, b2) => b2[1] - a[1]).slice(0, 14)
    .map(([k, n]) => ({ rgb16: k, approx: k.split(',').map((x) => (Number(x) << 4) + 8).join(','), n }));
