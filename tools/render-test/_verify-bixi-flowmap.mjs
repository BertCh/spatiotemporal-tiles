// Browser verification for the BIXI OD flowmap demo (FlowmapLayer).
// Loads /demo/bixi-flowmap, confirms the WebGL canvas draws, samples it across
...
// Run with the showcase dev server up:  STT_BASE_URL=http://localhost:3001 node _verify-bixi-flowmap.mjs
import { chromium } from "playwright";

const BASE = process.env.STT_BASE_URL ?? "http://localhost:3001";
const OUT = "output/bixi-flowmap-verify";
...
page.on("response", (r) => {
  const u = r.url();
  if (u.includes("/data/bixi-flowmap/")) {
    const key = u.split("/data/bixi-flowmap/")[1].split("?")[0];
...
console.log(`→ ${BASE}/demo/bixi-flowmap`);
await page.goto(`${BASE}/demo/bixi-flowmap`, { waitUntil: "domcontentloaded", timeout: 30000 });
