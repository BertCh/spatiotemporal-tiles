...
const DATASETS = ONLY
  ? ONLY.split(',')
  : ['satellites', 'satellite-trips-flat'];
...
for (const id of DATASETS) {
  ...
  try {
    await page.goto(`${BASE_URL}/demo/${id}`, {
    ...
    await page.screenshot({
      path: path.join(OUTPUT, `${id}.${TAG}.paused.png`),
    });
    // start playback
    await page.evaluate(() => {
    ...
    await page.screenshot({
      path: path.join(OUTPUT, `${id}.${TAG}.playing.png`),
    });
    console.log(
      `${id}: ok (errors ${errs.length})`,
      errs.slice(0, 3).join(' | '),
    );
  } catch (e) {
