...
try {
  ...
  result = await page.evaluate(() => {
    ...
    let cool = 0, warm = 0, mid = 0, lit = 0;
    // Background is dark slate (~[36,39,48]); count clearly-lit track pixels.
    for (let i = 0; i < px.length; i += 4) {
      const r = px[i], g = px[i + 1], b = px[i + 2];
      const bright = r + g + b;
      const bgish = Math.abs(r - 36) < 22 && Math.abs(g - 39) < 22 && Math.abs(b - 48) < 22;
      if (bright < 90 || bgish) continue; // skip background / near-black
      lit++;
      if (b > r + 25) cool++;        // blue-dominant → cold SST
      else if (r > b + 25) warm++;   // red/orange-dominant → warm SST
