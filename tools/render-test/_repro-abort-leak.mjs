...
await page.addInitScript(() => {
  ...
  window.addEventListener('unhandledrejection', (e) => {
    ...
    globalThis.__rejections.push({
      name: r?.name, message: String(r?.message ?? r).slice(0, 200),
...
await cdp.send('Network.emulateNetworkConditions', {
  ...
  downloadThroughput: (8 * 1024 * 1024) / 8, uploadThroughput: (1024 * 1024) / 8,
});
await page.goto(`${BASE}/demo/ocean-drifters`, { waitUntil: 'domcontentloaded', timeout: 60_000 });
await page.locator('.map-viewport').first().waitFor({ state: 'visible', timeout: 30_000 });
await page.waitForTimeout(8000);
const setRange = (v) => page.locator('input[type="range"]').first().evaluate((el, val) => {
  const setter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value')?.set;
  setter?.call(el, String(val));
  el.dispatchEvent(new Event('input', { bubbles: true }));
  el.dispatchEvent(new Event('change', { bubbles: true }));
}, v);
const b = await page.locator('input[type="range"]').first().evaluate((el) => ({ min: Number(el.min), max: Number(el.max) }));
