...
async function newPage() {
  ...
  await page.addInitScript(() => { globalThis.__sttProbe = { enabled: true }; });
  const errs = [];
...
const setRange = (page, v) => page.locator('input[type="range"]').first().evaluate((el, val) => {
  const setter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value')?.set;
  setter?.call(el, String(val));
  el.dispatchEvent(new Event('input', { bubbles: true }));
  el.dispatchEvent(new Event('change', { bubbles: true }));
}, v);
const bounds = (page) => page.locator('input[type="range"]').first().evaluate((el) => ({ min: Number(el.min), max: Number(el.max) }));
const rangeVal = (page) => page.locator('input[type="range"]').first().evaluate((el) => Number(el.value));
...
const drainPlayback = (page) => page.evaluate(() => {
...
const open = async (page, url) => {
  ...
  await page.locator('.map-viewport, canvas').first().waitFor({ state: 'visible', timeout: 30_000 });
};
const report = (name, obj) => console.log(`\n=== ${name} ===\n` + JSON.stringify(obj, null, 2));
...
const scenarios = {
  ...
  async probe() {
    ...
    report('probe-playback (e)', {
      eventCount: events.length, kinds, states, sample,
  ...
  async category() {
    ...
    const px = await page.evaluate(() => {
      const c = document.querySelector('.map-viewport canvas') || document.querySelector('canvas');
      ...
      const palette = {
        bird: [79, 195, 247], mammal: [255, 138, 101], fish: [77, 182, 172],
        reptile: [174, 213, 129], insect: [255, 213, 79],
      };
      ...
      const pn = Object.fromEntries(Object.entries(palette).map(([k, v]) => [k, sub(v)]));
      const counts = Object.fromEntries(Object.keys(palette).map((k) => [k, 0]));
      const bright = Object.fromEntries(Object.keys(palette).map((k) => [k, []]));
      let lit = 0;
      for (let i = 0; i < buf.length; i += 4) {
        const r = buf[i], g = buf[i + 1], b = buf[i + 2];
        if (Math.max(r, g, b) < 60) continue;
        if (Math.abs(r - bg[0]) < 18 && Math.abs(g - bg[1]) < 18 && Math.abs(b - bg[2]) < 18) continue;
        lit++;
        const u = sub([r, g, b]);
        let best = null, bestDot = 0.97;
        for (const [k, v] of Object.entries(pn)) {
          const dot = u[0] * v[0] + u[1] * v[1] + u[2] * v[2];
          if (dot > bestDot) { best = k; bestDot = dot; }
        }
        if (best) {
          counts[best]++;
      ...
      const classes = Object.entries(counts).filter(([, n]) => n > 60).map(([k, n]) => {
        const v = bright[k].sort((a, b2) => a - b2);
        return { type: k, n, p10: v[Math.floor(v.length * 0.1)], p90: v[Math.floor(v.length * 0.9)] };
      });
      return { w, h, lit, classes };
    });
    const fadeSpread = (px.classes ?? []).some((c2) => c2.p90 - c2.p10 > 50);
    report('category-fades (b)', {
      ...px, fadeSpread, screenshot: shot,
      pageErrors: errs.slice(0, 5),
  ...
  async refocus() {
    ...
    const sample = () => page.locator('input[type="range"]').first().evaluate((el) => ({
      v: Number(el.value), p: performance.now(),
    }));
    ...
    report('tab-refocus (c)', {
      simPerMs, realElapsed, unfrozenReal, simAdvanceMs,
      excessMs, clampTarget: 250, noFixWouldBe: frozenMs,
      pageErrors: errs.slice(0, 5),
  ...
  async loop() {
    ...
    await page.evaluate(() => {
      ...
      globalThis.__loopTimer = setInterval(() => {
        ...
        globalThis.__loopSamples.push({ v: el ? Number(el.value) : NaN, p: performance.now() });
      }, 500);
    });
    await clickBtn(page, /^▶$/);
    ...
    const wrapIdx = samples.findIndex((v, i2) => i2 > 0 && samples[i2 - 1] - v > span * 0.5);
    const post = wrapIdx >= 0 ? samples.slice(wrapIdx) : [];
    // Freeze detection: longest run of identical post-wrap samples.
    let frozenMax = 0, run = 0;
    for (let i = 1; i < post.length; i++) {
      run = post[i] === post[i - 1] ? run + 1 : 0;
      frozenMax = Math.max(frozenMax, run);
    }
    const states = events.map((e) => `${e.event}:${e.state}${e.degraded ? ':degraded' : ''}`);
    const sawSeeking = events.some((e) => e.state === 'seeking' || e.state === 'buffering' || e.event === 'waiting');
    ...
    report('drifters-loop (a)', {
      bounds: b, wrapped: wrapIdx >= 0, wrapIdx,
      trace: samples.filter((_, i2) => i2 % 8 === 0).map(pct),
      preWrapTail: samples.slice(Math.max(0, wrapIdx - 3), wrapIdx + 3).map(pct),
      postWrapHead: post.slice(0, 8).map(pct),
      frozenMaxRunMs: frozenMax * 500,
      sawSeeking, movedAfter, lastQoe,
      stateTimeline: states.slice(0, 40),
      screenshot: shot, pageErrors: errs.slice(0, 5),
...
for (const name of run) {
  ...
  catch (e) { report(`${name} (CRASHED)`, { error: String(e?.message || e).slice(0, 300) }); }
}
await browser.close();
