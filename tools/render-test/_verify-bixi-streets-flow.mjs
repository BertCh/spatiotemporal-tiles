// Browser verification for the BIXI directional street-flow demo
// (FlowCorridorLayer + ChevronFlowExtension). Loads /demo/bixi-streets-flow,
// confirms the WebGL canvas draws, samples it across playback to prove the
// render changes (the matrix brightness AND the marching chevrons both animate),
...
for (let i = 0; i < 4; i++) {
  ...
  await page.waitForTimeout(2500); // let the chevrons march / playhead advance
}
