...
pub fn run(args: Args) -> Result<()> {
    ...
    if let Some(ref intermediate_file) = args.from_intermediate {
        println!(
            "📂 Using existing intermediate file: {}",
    ...
    let trips = if let Some(max) = args.max_trips {
        ...
        if args.chronological {
            ...
            if let (Some(first), Some(last)) = (sampled.first(), sampled.last()) {
                println!(
                    "   📊 Selected first {} trips chronologically (from {})",
                    sampled.len(),
                    trips.len()
                );
                println!(
...
fn flows_from_paths_intermediate(args: &Args, paths_parquet: &PathBuf) -> Result<()> {
    ...
    println!(
        "\n🔁 Aggregating paths intermediate onto OSM streets ({} flow bins)",
        args.flow_bin
    );
    let mut agg = FlowAggregator::new(bin_ms, network);
    let rows =
        crate::datasets::nyc_rideshare_flows::aggregate_paths_parquet(paths_parquet, &mut agg)?;
    let (added, skipped, exact, fallback, miss) = agg.stats();
    let matched = exact + fallback;
    let total = (matched + miss).max(1);
    println!(
        "   ✓ {} rows read ({} trips aggregated, {} skipped)",
        rows, added, skipped
    );
    println!(
...
fn generate_flows_parquet(trips: &[Trip], args: &Args, output: &PathBuf) -> Result<()> {
    ...
    println!(
        "✓ Routing complete, aggregating into {} flow bins...",
        args.flow_bin
    );
    println!(
    ...
    println!(
        "✓ GeoParquet (flow corridors) written: {} features across {} bins",
        features, bins
    );
    Ok(())
}

/// Fixed seed for synthetic-trip generation. A deterministic RNG keeps builds
/// byte-reproducible (a project value): identical CLI args produce identical
/// output. `thread_rng` here made every run diverge.
const SYNTHETIC_TRIP_SEED: u64 = 0x5454_5f52_4944_45; // "TT_RIDE"

fn generate_synthetic_trips(args: &Args) -> Result<Vec<Trip>> {
    let mut rng = StdRng::seed_from_u64(SYNTHETIC_TRIP_SEED);
...
fn download_tlc_data(month: &str, verbose: bool) -> Result<PathBuf> {
    ...
    if !response.status().is_success() {
        return Err(anyhow!(
            "Failed to download TLC data: HTTP {}",
            response.status()
        ));
    }
    ...
    use std::io::Read;
    let pb = ProgressBar::new(if total_size > 0 {
        total_size
    } else {
        2_000_000_000
    }); // 2GB estimate if unknown
    pb.set_style(
        ProgressStyle::default_bar()
...
fn parse_tlc_csv(path: &PathBuf, args: &Args) -> Result<Vec<Trip>> {
    ...
    for result in csv_reader.deserialize() {
        ...
        let estimated_distance =
            common::haversine_distance(pickup_lat, pickup_lon, dropoff_lat, dropoff_lon);
...
fn interpolate_route(
    ...
    for i in 0..=num_intervals {
        ...
        let segment_end_dist = cumulative_distances
            .get(segment_idx + 1)
            .copied()
...
fn interpolate_route_with_vertex_times(
    ...
    for k in 0..=n_steps {
        ...
        let ts_dt = chrono::DateTime::<Utc>::from_timestamp_millis(ts)
            .unwrap_or(start_time_anchor(start_ms));
...
fn generate_trajectories_parquet(trips: &[Trip], args: &Args, output: &PathBuf) -> Result<()> {
    ...
    let mut property_columns = vec![
        ...
        PropertyColumn {
            name: "passenger_count".into(),
            kind: PropertyKind::Numeric,
        },
        PropertyColumn {
            name: "trip_distance".into(),
            kind: PropertyKind::Numeric,
        },
        PropertyColumn {
    ...
    if args.with_bearing {
        property_columns.push(PropertyColumn {
            name: "bearing".into(),
            kind: PropertyKind::Numeric,
        });
    }
    ...
    pb.finish_and_clear();
    println!(
        "📊 Generated {} trajectory points (wrote {} rows)",
        total_points, rows
    );
    if failed_routes > 0 {
...
fn generate_paths_geojson(trips: &[Trip], args: &Args, output: &PathBuf) -> Result<()> {
    ...
    pb.finish_and_clear();
    println!(
        "📊 Generated {} path features with {} total coordinates",
        features.len(),
        total_coords
    );
    if failed_routes > 0 {
...
fn route_trips_parallel(trips: &[Trip], args: &Args) -> (Vec<TripResult>, usize, usize) {
    ...
    println!(
        "   Processing {} batches of {} trips each\n",
...
fn generate_od_parquet(trips: &[Trip], args: &Args, output: &PathBuf) -> Result<()> {
    ...
    let mut property_columns = vec![
        ...
        PropertyColumn::numeric("fare_amount"),
        PropertyColumn {
            name: "pickup_borough".into(),
            kind: PropertyKind::String,
        },
        PropertyColumn {
            name: "dropoff_borough".into(),
            kind: PropertyKind::String,
        },
    ];
    if args.with_bearing {
    ...
    if skipped > 0 {
        println!(
            "⚠️  {} zero-length (pickup == dropoff) trips skipped",
            skipped
        );
    }
...
mod tests {
    ...
    fn bearing_diagonal_northeast() {
        ...
        assert!(
            b < 45.0,
            "NE diagonal bearing should be just under 45°, got {b}"
        );
    }
    ...
    fn point_bearings_last_repeats_previous() {
        ...
        let pts = vec![(-74.0, 40.0, t), (-74.0, 41.0, t), (-73.0, 41.0, t)];
    ...
    /// Synthetic trip generation must be byte-reproducible: two runs with the
    /// same args produce identical trips (regression for the unseeded
    /// `thread_rng` that broke reproducible builds).
    #[test]
    fn synthetic_trips_are_deterministic() {
        use clap::Parser;
        let mut args = Args::parse_from(["nyc-rideshare"]);
        args.num_trips = 25;

        let a = generate_synthetic_trips(&args).expect("gen a");
        let b = generate_synthetic_trips(&args).expect("gen b");

        assert_eq!(a.len(), b.len(), "trip counts differ between runs");
        for (x, y) in a.iter().zip(b.iter()) {
            assert_eq!(x.pickup_time, y.pickup_time);
            assert_eq!(x.dropoff_time, y.dropoff_time);
            assert_eq!(x.pickup_lon.to_bits(), y.pickup_lon.to_bits());
            assert_eq!(x.pickup_lat.to_bits(), y.pickup_lat.to_bits());
            assert_eq!(x.dropoff_lon.to_bits(), y.dropoff_lon.to_bits());
            assert_eq!(x.dropoff_lat.to_bits(), y.dropoff_lat.to_bits());
            assert_eq!(x.fare_amount.to_bits(), y.fare_amount.to_bits());
            assert_eq!(x.passenger_count, y.passenger_count);
        }
    }
