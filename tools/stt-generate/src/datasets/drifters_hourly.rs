//! EXPERIMENTAL: generate ocean-current trajectories from NOAA's Global
//! Drifter Program **hourly** product (Elipot et al.).
//!
//! This is a parallel experiment alongside the production `drifters` generator
//! (the 6-hourly `gdp_interpolated_drifter` product). It is intentionally
//! self-contained so the existing 6-hourly path is left untouched.
//!
//! Data source: AOML GDP ERDDAP `drifter_hourly_qc` — the 1-hour
//! quality-controlled, interpolated product of satellite-tracked surface
//! drifting buoys (Oct 1987 → Oct 2022), v2.01. Public domain (CC0).
//!   <https://erddap.aoml.noaa.gov/gdp/erddap/tabledap/drifter_hourly_qc.html>
//!   Elipot, Sykulski, Lumpkin, Centurioni & Pazos (2022).
//!
//! Two schema differences vs. the 6-hourly product drive the only behavioural
//! changes here:
//!   * the temperature column is `sst` (not `temp`), and
//!   * it is reported in **Kelvin** (not °C) — we convert to °C on ingest so the
//!     downstream temp-band / SST colour ramp is identical to the 6-hourly path.
//!
//! Each drifter's hourly fixes are grouped into a track, split at large time
//! gaps and antimeridian crossings, and emitted as LineStrings with per-vertex
//! real timestamps so the deck.gl trips layer animates the buoy drifting along
...
#[derive(Parser, Debug)]
#[command(about = "EXPERIMENTAL: Global Drifter Program HOURLY ocean-current trajectories")]
pub struct Args {
    /// Output file (.stt or .parquet)
    #[arg(short, long, default_value = "drifters-hourly.stt")]
    pub output: PathBuf,

    /// Inclusive start date (YYYY-MM-DD). Must be >= 1987-10-02 and <= 2022-10-31.
    ...
    /// Temporal bucket size (e.g. "1d", "7d"). Hourly fixes are ~6× denser than
    /// the 6-hourly product, so coarsen this for long spans to keep the
    /// z0×time-bucket tile count (and per-frame load) manageable.
    #[arg(long, default_value = "1d")]
    pub temporal_bucket: String,

    /// Cache directory for the downloaded monthly CSVs.
    #[arg(long, default_value = "data/gdp-hourly-cache")]
...
pub fn run(args: Args) -> Result<()> {
    println!("🌊 Global Drifter Program Generator — HOURLY (experimental)");
    println!("==========================================================\n");
    ...
    // Cache key must encode the bounds, or a bounded run (e.g. a smoke test)
    // and a later global run collide on the same monthly file and the global
    // run silently reuses the bounded subset.
    ...
    for id in drifter_ids {
        ...
        for (seg_idx, seg) in segments.iter().enumerate() {
            ...
            // hourly record are gap-filled (forward then backward) so every
    ...
    println!("\n✅ Hourly drifter generation complete!");
...
fn build_url(start: &str, end: &str, bounds: Option<(f64, f64, f64, f64)>) -> String {
    // Hourly schema uses `sst` (Kelvin) where the 6-hourly product used `temp`.
    let mut parts: Vec<String> = vec![
        erddap_encode("ID,WMO,time,longitude,latitude,sst,ve,vn"),
...
fn parse_csv(
    ...
    let (ci_id, ci_wmo, ci_time, ci_lon, ci_lat, ci_sst) = (
    ...
    for rec in rdr.records() {
        ...
        // `sst` is fitted sea-water temperature in Kelvin → convert to °C, and
        // drop anything outside a plausible ocean range (guards against fill
        // sentinels surviving the finite check, e.g. 0 K → −273 °C).
        let temp = ci_sst
            .and_then(|i| rec.get(i))
            .and_then(parse_f64)
            .map(|k| k - KELVIN_TO_CELSIUS)
            .filter(|c| (-5.0..=45.0).contains(c));
