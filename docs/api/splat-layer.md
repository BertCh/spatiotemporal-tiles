...
- **Two-level cache, split by what a prop change actually invalidates**:
  - a **prepared-tile cache**, keyed by tile + a _style_ digest (`quaternionColumn`, `scaleColumn`, `colorColumn`, `elevationProperty`) — these choose which raw columns get bound, so changing one rebuilds the tile's attribute set;
