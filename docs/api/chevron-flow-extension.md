...
| `uniformSpacing` | `boolean` | `false` | Fits a whole number of chevrons into each rendered path *segment* instead of using the raw `period` (which is measured per-segment and restarts at every vertex), so arrows are never truncated at a joint. Intended for static chevrons (`speed: 0`); combined with a nonzero `speed`, the march steps at each joint. |
| `directionColor` | `boolean` | `false` | Tints the arrowheads by the compass bearing they point, using four colors placed at the cardinal directions (`directionColors`) and blended cyclically around the compass. Applied to the arrowheads only. |
| `directionColors` | `[Color, Color, Color, Color]` | amber/green/teal/violet | The four cardinal colors `[N, E, S, W]`, each `[r, g, b]` in 0–255, placed at bearings `offset`, `offset + 90`, `offset + 180`, `offset + 270` and interpolated cyclically for in-between bearings. |
...
The shader-injection object returned by `getShaders()` is memoized per extension instance — deck.gl calls `getShaders()` on every sublayer construction, and a fresh object each time would thrash the shader/pipeline cache. Because a `LayerExtension` instance's options are compared by value (not reference) when keying the sublayer cache, constructing a fresh `new ChevronFlowExtension(opts)` on every render is free as long as `opts` is stable. Per-frame updates are limited to the one small uniform block described above; use `getTime` rather than `currentTime` so the host layer instance itself stays cached across animation ticks.
...
- `uniformSpacing` is designed for static chevrons (`speed: 0`); with a nonzero `speed` the march visibly steps at each segment joint, because the fitted period differs per segment.
- `directionColors` and other option values are read once per extension construction; changing them requires constructing a new `ChevronFlowExtension`, not mutating an existing instance's props.
