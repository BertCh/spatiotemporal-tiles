...
| `--format-version <1\|2>`    | `2`     | Packed format version to emit. See below.                                |
...
`--format-version 2` (the default) embeds schema templates in the manifest,
killing the per-tile schema tax, and emits sectioned layer frames with
`TILE_META`, time-sorted rows, and `STTP`/`STTD` object magic (packed spec §5.2,
§9.2). `1` is the transitional kill switch: it reproduces the frozen 0.3.x writer
byte for byte (golden-pinned), so a v2 regression can be rolled back by
rebuilding. It is also what `stt-serve` byte-parity checks build against, since
serve always emits v1 frames. v1 emission is kept for one release, then removed.
