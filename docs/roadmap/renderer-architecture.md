...
| **time-filter window**  | `timeWindow` (**full** width, ms)                                                                         | `timeWindow` (**full**) **or** `windowHalf` (**half**, default `250`) | `timeWindow` (**full** width, ms)                | **Bridged** by `resolveTimeWindow` (`packages/three/src/lib/time-window.ts`): every three layer now ALSO accepts the deck/maplibre full-width `timeWindow` (→ `windowHalf = timeWindow / 2`), so copying `timeWindow: 86_400_000` from a deck demo is honored, not silently dropped to `windowHalf = 250`. The three-native `windowHalf` stays a lower-level alias and wins when both are set. _(Was a 2× magnitude trap before the bridge.)_ |
| fade-in ramp            | `fadeInDuration`                                                                                          | `fadeInDuration` (parity) or native `fadeIn`                   | `fadeInDuration`                                 | **Bridged** by `resolveTimeWindow`: three now ALSO accepts the deck/maplibre `fadeInDuration` (native `fadeIn` wins when both set). The box layer already used the deck/maplibre name.        |
| fade-out ramp           | `fadeOutDuration`                                                                                         | `fadeOutDuration` (parity) or native `fadeOut`                | `fadeOutDuration`                                | **Bridged** by `resolveTimeWindow`: `fadeOutDuration` accepted (native `fadeOut` wins) — same bridge as fade-in.                                                                              |
