...
const { ORDERS, ORDER_KEYS, GESTURE_KEYS, INTERACTIONS, CUMULATIVE, STATS, SEEKS, CELLS, sceneAt, Cube, ByteStrip } =
  __test;
...
describe("SpaceTimeCurve data model", () => {
  ...
  it("each gesture touches the tiles the story claims", () => {
    const distinct = Object.fromEntries(GESTURE_KEYS.map((g) => [g, CUMULATIVE[g].length]));
    expect(distinct).toEqual({ play: 16, pan: 16, panPlay: 16, zoom: 28, preload: 4 });
  });
  ...
  it("a gesture's cumulative set is exactly the union of its frames", () => {
    for (const g of GESTURE_KEYS) {
      ...
      expect(new Set(CUMULATIVE[g].map((c) => `${c.x},${c.y},${c.t}`))).toEqual(union);
    }
  });
  ...
  it("range-read matrix matches the locked story", () => {
    const matrix = Object.fromEntries(
      ORDER_KEYS.map((k) => [k, GESTURE_KEYS.map((g) => STATS[k][g].runs.length)]),
    );
  ...
  it("each specialist walk wins its own gesture, and hilbert3 is never worst", () => {
    const runs = (k: (typeof ORDER_KEYS)[number], g: (typeof GESTURE_KEYS)[number]) => STATS[k][g].runs.length;
    ...
    for (const g of GESTURE_KEYS) {
      ...
      expect(runs("hilbert3", g)).toBeLessThanOrEqual(Math.max(runs("spatial", g), runs("time", g)));
    }
  });
      ...
      for (const gesture of GESTURE_KEYS) {
        for (const p of progresses) {
          ...
          for (const animating of [true, false]) {
            expect(() =>
              renderToString(React.createElement(Cube, { order, gesture, active, touched, animating })),
            ).not.toThrow();
          }
          expect(() =>
            renderToString(React.createElement(ByteStrip, { order, gesture, active, touched })),
          ).not.toThrow();
        }
        ...
        expect(() =>
          renderToString(
            React.createElement(Cube, {
              ...
              touched: new Set(CUMULATIVE[gesture].map((c) => `${c.x},${c.y},${c.t}`)),
              animating: false,
            }),
          ),
        ).not.toThrow();
