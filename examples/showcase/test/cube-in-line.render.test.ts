...
import { profileIdFromUrl, type DensityProfile } from "../src/lib/densityProfile.ts";
...
describe("CubeInLineView", () => {
  it("renders the laid-down line, heatmap, walk labels and the auto anchor", () => {
    const html = renderToString(React.createElement(CubeInLineView, { profile: fixture() }));
  ...
  it("flags when the auto heuristic disagrees with the measured best", () => {
    ...
    const html = renderToString(
      React.createElement(CubeInLineView, {
        profile: fixture({ autoChoice: "hilbert3", nativeSeeks: { spatial: 3, hilbert3: 20, morton3: 40, time: 30 } }),
      }),
    );
  ...
  it("profileIdFromUrl pulls the archive stem from a manifest url", () => {
    expect(profileIdFromUrl("/data/nyc-rideshare/manifest.json")).toBe("nyc-rideshare");
    expect(profileIdFromUrl("/data/argoverse-1/lidar/manifest.json")).toBe("argoverse-1");
  ...
  it("does not throw for any dataset id / autoChoice combination", () => {
    for (const id of ["drifters", "nyc-rideshare", "gtfs-nl", "flights", "unknown"]) {
      for (const autoChoice of ["spatial", "hilbert3", "morton3", "time"] as const) {
        expect(() =>
          renderToString(React.createElement(CubeInLineView, { profile: fixture({ id, autoChoice }) })),
        ).not.toThrow();
      }
    }
  });
