/**
 * Standard video-player keyboard map for the fullscreen demo viewer
 ...
 * behavior is handled by TimeControls.
 *
 * Mounted ONLY by the fullscreen demo page: the scrolling embed pages must
 * not capture window keys (Space there means "scroll the page").
 */
import { useEffect, useRef } from "react";
import type { DemoPlayback } from "./useDemoPlayback";

/**
 * Same ladder as @stt/deck.gl auto-speed DEFAULT_STEPS (and the TimeControls
 * preset row), so keyboard stepping only lands on speeds the UI can display.
 */
