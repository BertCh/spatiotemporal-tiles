...
const TileAnatomy: React.FC = () => (
  ...
  >
      ...
      <div className="flex items-stretch gap-2">
        {COLS.map((c) => (
          <div
            ...
            style={{
              background: c.required ? "var(--surface)" : "var(--surface-sunken)",
              border: "1px solid var(--hairline)",
          ...
          >
            ...
            <span className="font-mono text-[10px] mt-0.5" style={{ color: "var(--accent)" }}>
              {c.type}
            </span>
            <span
    ...
    <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
      {/* coordinates */}
      <div className="rounded-md p-3" style={{ background: "var(--surface-sunken)", border: "1px solid var(--hairline)" }}>
        <span className="eyebrow" style={{ fontSize: 9 }}>coordinates</span>
        <svg viewBox="0 0 180 90" className="w-full mt-2" role="img" aria-label="Diagram: a point snapping to one world-anchored integer grid shared by every tile.">
          {Array.from({ length: 9 }, (_, i) => (
            <React.Fragment key={i}>
              <line x1={10 + i * 20} y1="6" x2={10 + i * 20} y2="86" stroke="var(--hairline)" strokeWidth="0.75" />
              {i < 5 ? <line x1="10" y1={6 + i * 20} x2="170" y2={6 + i * 20} stroke="var(--hairline)" strokeWidth="0.75" /> : null}
            </React.Fragment>
          ))}
          <circle cx="97" cy="53" r="3.5" fill="var(--ink-400)" />
          <path d="M 97 53 L 89 47" stroke="var(--accent)" strokeWidth="1.25" markerEnd="none" />
          <circle cx="90" cy="46" r="3" fill="var(--accent)" />
        </svg>
        <p className="font-mono text-[10px] mt-2" style={{ color: "var(--ink-700)" }}>
          lon = x0 + q · sx
        </p>
        <p className="text-[10.5px] mt-1 leading-snug" style={{ color: "var(--ink-500)" }}>
        ...
        <span className="inline-block mt-2 rounded px-1.5 py-0.5 font-mono text-[10px]" style={{ background: "var(--accent-soft)", color: "var(--accent)" }}>
          f64 8 B → int 4 B
        </span>
      </div>
      ...
      {/* vertex times */}
      <div className="rounded-md p-3" style={{ background: "var(--surface-sunken)", border: "1px solid var(--hairline)" }}>
        <span className="eyebrow" style={{ fontSize: 9 }}>vertex times</span>
        <svg viewBox="0 0 180 90" className="w-full mt-2" role="img" aria-label="Diagram: vertex timestamps stored as small step counts from a per-tile origin.">
          <line x1="10" y1="60" x2="170" y2="60" stroke="var(--ink-400)" strokeWidth="1" />
          {Array.from({ length: 9 }, (_, i) => (
            <line key={i} x1={14 + i * 19} y1="55" x2={14 + i * 19} y2="65" stroke="var(--hairline)" strokeWidth="1" />
          ))}
          <path d="M 14 60 L 14 34" stroke="var(--accent)" strokeWidth="1.25" />
          <text x="14" y="26" fontSize="9" textAnchor="middle" fill="var(--accent)" fontFamily="monospace">origin</text>
          {[2, 3, 5, 8].map((k) => (
            <circle key={k} cx={14 + k * 19} cy="60" r="3" fill="var(--accent)" />
          ))}
          <text x="90" y="82" fontSize="9" textAnchor="middle" fill="var(--ink-400)" fontFamily="monospace">step = 250 ms</text>
        </svg>
        <p className="font-mono text-[10px] mt-2" style={{ color: "var(--ink-700)" }}>
          t = origin + q · step
        </p>
      ...
      {/* attributes */}
      <div className="rounded-md p-3" style={{ background: "var(--surface-sunken)", border: "1px solid var(--hairline)" }}>
        <span className="eyebrow" style={{ fontSize: 9 }}>attributes</span>
        <svg viewBox="0 0 180 90" className="w-full mt-2" role="img" aria-label="Diagram: a float value range mapped onto 65536 integer levels.">
          <rect x="10" y="24" width="160" height="14" rx="7" fill="none" stroke="var(--ink-400)" />
          <text x="10" y="16" fontSize="9" fill="var(--ink-400)" fontFamily="monospace">min</text>
          <text x="170" y="16" fontSize="9" textAnchor="end" fill="var(--ink-400)" fontFamily="monospace">max</text>
          {Array.from({ length: 17 }, (_, i) => (
            <line key={i} x1={12 + i * 9.75} y1="42" x2={12 + i * 9.75} y2="48" stroke="var(--hairline)" strokeWidth="1" />
          ))}
          <text x="90" y="64" fontSize="9" textAnchor="middle" fill="var(--ink-400)" fontFamily="monospace">65 536 levels across the range</text>
