...
const FileTree: React.FC = () => (
  <Card>
    ...
    <div className="mt-3 space-y-1.5">
      {TREE.map((row) => (
        <div key={row.tree} className="flex flex-wrap items-baseline gap-x-3 gap-y-0.5">
          <span
          ...
          {row.badge ? <Badge tone={row.tone ?? "cool"}>{row.badge}</Badge> : null}
          {row.note ? (
    ...
    <p
      ...
      style={{ color: "var(--ink-400)", borderTop: "1px solid var(--hairline)" }}
    >
...
const LEAVES: Leaf[] = [
  { id: "leaf 0", zooms: "z 5–9", bbox: { x: 1, y: 1, w: 9, h: 12 }, time: [0, 24], verdict: "skip", reason: "wrong place" },
  { id: "leaf 1", zooms: "z 5–9", bbox: { x: 13, y: 1, w: 10, h: 12 }, time: [0, 12], verdict: "fetch" },
  { id: "leaf 2", zooms: "z 5–9", bbox: { x: 13, y: 1, w: 10, h: 12 }, time: [12, 24], verdict: "skip", reason: "wrong time" },
  { id: "leaf 3", zooms: "z 10–14", bbox: { x: 1, y: 1, w: 22, h: 12 }, time: [0, 24], verdict: "skip", reason: "wrong zoom" },
];

const BboxMini: React.FC<{ leaf: Leaf; active: boolean }> = ({ leaf, active }) => (
  <svg viewBox="0 0 24 14" width="34" height="20" aria-hidden="true">
    <rect x="0.5" y="0.5" width="23" height="13" rx="1.5" fill="none" stroke="var(--hairline)" />
    <rect
      x={leaf.bbox.x}
      y={leaf.bbox.y}
      width={leaf.bbox.w}
...
const TimeMini: React.FC<{ leaf: Leaf; active: boolean }> = ({ leaf, active }) => (
  <svg viewBox="0 0 60 8" width="66" height="9" aria-hidden="true">
    <rect x="0.5" y="1.5" width="59" height="5" rx="2.5" fill="none" stroke="var(--hairline)" />
    <rect
    ...
    <div className="mt-3 space-y-1.5">
      {LEAVES.map((leaf) => {
        ...
        return (
          ...
          >
            <span className="font-mono text-[11px] w-12" style={{ color: "var(--ink-700)" }}>
              {leaf.id}
            </span>
            <span className="font-mono text-[11px] w-16" style={{ color: "var(--ink-500)" }}>
              {leaf.zooms}
            </span>
...
const DIR_ROWS: DirEntryRow[] = [
  ...
  { key: "z7 · h2841 · 10:00", count: "214" },
  {
    key: "z7 · h2841 · 11:00",
    count: "890",
    run: { span: 1, label: "run ×1 → pack 0 @ 41 kB, 96 kB", note: "offset stores 0 — it follows the previous blob" },
  },
  {
    key: "z7 · h2842 · 08:00",
    count: "3 102",
    run: { span: 1, label: "run ×1 → pack 0 @ 137 kB, 210 kB", note: "next Hilbert cell — still sequential" },
  },
  {
    key: "z7 · h2842 · 09:00",
    count: "2 977",
...
const DirectoryInternals: React.FC = () => (
  <Card>
    <SubHead>Deeper: inside the directory — sorted keys plus run-length encoding</SubHead>
    <p className="mt-1 text-[11px]" style={{ color: "var(--ink-500)" }}>
        ...
        <table className="w-full min-w-[520px] border-collapse text-[11px]">
          <thead>
            <tr style={{ color: "var(--ink-400)" }}>
              <th className="text-left font-medium pb-1.5 pr-3">entry key (delta-coded)</th>
              <th className="text-right font-medium pb-1.5 pr-4">features</th>
              <th className="text-left font-medium pb-1.5">blob run (written once per run)</th>
            </tr>
          </thead>
          <tbody>
            {DIR_ROWS.map((row) => (
              <tr key={row.key} style={{ borderTop: "1px solid var(--hairline)" }}>
                <td className="py-1.5 pr-3 font-mono" style={{ color: "var(--ink-700)" }}>
                  {row.key}
                </td>
                <td className="py-1.5 pr-4 font-mono text-right" style={{ color: "var(--ink-500)" }}>
                  {row.count}
                </td>
                {row.run ? (
                  <td
                    ...
                    style={{
                      ...
                      background: row.run.dedup ? "rgba(10,119,144,0.05)" : undefined,
                    }}
                  >
                    <span className="font-mono block" style={{ color: row.run.dedup ? "var(--accent)" : "var(--ink-700)" }}>
                      {row.run.label}
                    </span>
                    {row.run.note ? (
                      <span className="block mt-0.5" style={{ color: "var(--ink-400)" }}>
                        {row.run.note}
                      </span>
                    ) : null}
                  </td>
    ...
    <div className="mt-3 pt-3 flex flex-wrap gap-1.5" style={{ borderTop: "1px solid var(--hairline)" }}>
      {DIR_CHIPS.map((c) => (
        <span
          key={c}
          className="rounded px-1.5 py-0.5 text-[10px]"
          style={{ background: "var(--surface-sunken)", color: "var(--ink-500)", border: "1px solid var(--hairline)" }}
        >
          {c}
        </span>
      ))}
  ...
  <Card>
    ...
    <div className="overflow-x-auto">
      ...
      >
        ...
        {/* dedup callout */}
        <text x="440" y="118" fontSize="10.5" fontWeight="600" fill="var(--accent)" textAnchor="middle">
          identical bytes → one blob (blake3)
        </text>

        {/* pack frame */}
        <text x="48" y={PACK_Y - 12} fontSize="11" fontFamily={MONO} fill="var(--ink-500)">
          packs/0e49f7a4….sttp
        </text>
        <rect
          x="48"
        ...
        <text x="828" y={PACK_Y + 24} fontSize="12" fill="var(--ink-400)" textAnchor="middle">
          …
        </text>
        ...
        <path
          ...
          strokeWidth="1.25"
        />
        <text x="409" y={PACK_Y + 76} fontSize="10.5" fontWeight="600" textAnchor="middle" fill="var(--accent)">
