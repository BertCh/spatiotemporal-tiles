  ...
  <Card>
    ...
    <div className="mt-3 flex flex-wrap items-center gap-2">
      {LOOP.map((n, i) => (
        <React.Fragment key={n.t}>
          ...
          <span
            ...
            style={{
              background: n.accent ? "var(--accent-soft)" : "var(--surface-sunken)",
      ...
      <span
        ...
        style={{ border: "1px dashed var(--hairline)", color: "var(--ink-400)" }}
      >
        <svg viewBox="0 0 24 24" width="12" height="12" aria-hidden="true">
          <path fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" d="M4 9a8 8 0 0 1 14-4l2 2M20 15a8 8 0 0 1-14 4l-2-2" />
        </svg>
        re-tune next build
      </span>
    </div>
    <p className="mt-3 text-[11px] leading-relaxed" style={{ color: "var(--ink-500)" }}>
      ...
      temporal bucket and runs the advisors; <span className="font-mono">--auto</span>{" "}
      folds the safe ones in; then <span className="font-mono">inspect</span>,{" "}
...
const MeasureFigure: React.FC = () => {
  ...
  return (
    <Card>
      <SubHead>Measure, don't guess — every projection is a real encode</SubHead>
      <div className="mt-3 flex flex-wrap items-center gap-2">
        {["source rows", "deterministic stride sample", "real stt-core encoder + zstd", "per-column bytes"].map(
          (t, i) => (
            <React.Fragment key={t}>
              ...
              <span
                ...
                style={{
                  background: i === 2 ? "var(--accent-soft)" : "var(--surface-sunken)",
...
const REVERSIBLE: Advice[] = [
  { flag: "--zstd-level 19", why: "decode-free win at rest", projected: "−14% (measured)", confidence: "high" },
  { flag: "--temporal-lod 1d@8,30d@4", why: "long-tail time span", projected: "fewer low-zoom fetches", confidence: "medium" },
  { flag: "--blob-ordering spatial", why: "time-deep, few cells", projected: "~3× fewer requests", confidence: "medium" },
];
const LOSSY: Advice[] = [
  { flag: "--quantize-coords 0.1", why: "coords dominate the bytes", projected: "−31% (measured)", confidence: "high" },
  { flag: "--quantize-attr", why: "raw f64 property columns", projected: "−9% (measured)", confidence: "medium" },
];
...
const ConfBadge: React.FC<{ c: Advice["confidence"] }> = ({ c }) => {
  ...
  return (
    <span className="rounded px-1 py-0.5 text-[9px] font-semibold uppercase tracking-wide" style={style}>
      {c}
    </span>
  );
};
...
const AdviceRow: React.FC<{ a: Advice; warm?: boolean }> = ({ a, warm }) => (
  ...
  >
    <div className="flex items-center justify-between gap-2">
      <span className="font-mono text-[11px] font-semibold" style={{ color: warm ? WARM_INK : "var(--accent)" }}>
        {a.flag}
      </span>
      <ConfBadge c={a.confidence} />
    </div>
    <div className="mt-1 flex flex-wrap items-baseline gap-x-2 text-[10.5px]" style={{ color: "var(--ink-500)" }}>
      <span>{a.why}</span>
      <span className="font-mono" style={{ color: warm ? WARM_INK : "var(--ink-700)" }}>
        · {a.projected}
      </span>
    </div>
  </div>
...
const FirewallFigure: React.FC = () => (
  <Card>
      ...
      <div className="flex flex-wrap gap-1.5">
        {ANALYZERS.map((a) => (
          <span
            ...
            style={{ background: "var(--surface-sunken)", border: "1px solid var(--hairline)", color: "var(--ink-700)" }}
          >
            {a}
          </span>
        ))}
    ...
    <div className="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-3">
      {/* reversible */}
      <div className="rounded-lg p-3" style={{ border: "1px solid var(--accent)", background: "var(--accent-soft)" }}>
        <div className="flex items-center gap-2">
          <span className="h-2 w-2 rounded-full" style={{ background: "var(--accent)" }} aria-hidden="true" />
          <span className="font-display text-[12px] font-semibold" style={{ color: "var(--accent)" }}>
        ...
        <p className="mt-1 text-[10.5px] leading-snug" style={{ color: "var(--ink-500)" }}>
          ...
          <span className="font-mono">--auto</span> and appended to the pasteable
          command, in advisor order.
        </p>
      ...
      {/* lossy */}
      <div className="rounded-lg p-3" style={{ border: `1px solid ${WARM}`, background: "var(--surface)" }}>
        <div className="flex items-center gap-2">
          <span className="h-2 w-2 rounded-full" style={{ background: WARM }} aria-hidden="true" />
          <span className="font-display text-[12px] font-semibold" style={{ color: WARM_INK }}>
            Lossy → surfaced only, never auto
          </span>
        </div>
        <p className="mt-1 text-[10.5px] leading-snug" style={{ color: "var(--ink-500)" }}>
          Degrades data. Reported loudly with its projection, but{" "}
          <em>you</em> add it by hand — never in{" "}
          <span className="font-mono">--auto</span>, never in the command.
        </p>
        <div className="mt-2.5 space-y-1.5">
          {LOSSY.map((a) => (
    ...
    <p className="mt-3 text-[11px] leading-relaxed" style={{ color: "var(--ink-500)" }}>
      This split is the project's <strong style={{ color: "var(--ink-700)" }}>no-thinning</strong>{" "}
      principle made mechanical: a build never silently throws data away to hit
      a size target. Quantization and per-tile budgets can shrink an archive a
      lot — so they stay a deliberate, per-dataset choice, and the default
      build is byte-identical whether or not you ran the analyzer.
    </p>
  </Card>
);
...
const FINDINGS: Finding[] = [
  { sev: "high", code: "expensive-feature-ids", smell: "near-incompressible hash-like ids dominate the bytes", fix: "sequential ids / drop" },
  { sev: "high", code: "raw-f64-column", smell: "plain Float64 property columns worth quantizing", fix: "--quantize-attr" },
  { sev: "high", code: "missing-summary-tier", smell: "huge point dataset with no aggregated overview", fix: "--summary-tier quadbin" },
  { sev: "med", code: "unpaged-large", smell: "whole-load directory on a large tile count", fix: 'layout: "paged"' },
  { sev: "med", code: "oversized-blobs", smell: "individual tiles past 1 MiB compressed", fix: "raise min-zoom / split" },
  { sev: "med", code: "z0-bomb", smell: "deep pyramid under a tiny geographic extent", fix: "clamp zoom range" },
  { sev: "med", code: "dead-columns", smell: "constant / all-null property columns", fix: "drop at source" },
];

const DoctorFigure: React.FC = () => (
  <Card>
    <SubHead>The doctor — severity-ranked findings on the built tileset</SubHead>
    <div className="mt-3 overflow-x-auto">
      <table className="w-full min-w-[560px] border-collapse text-[11px]">
        <thead>
          <tr style={{ color: "var(--ink-400)" }}>
            <th className="text-left font-medium pb-1.5 pr-2">sev</th>
            <th className="text-left font-medium pb-1.5 pr-3">finding (stable code)</th>
            <th className="text-left font-medium pb-1.5 pr-3">what it smells</th>
            ...
            <tr key={f.code} style={{ borderTop: "1px solid var(--hairline)" }}>
              <td className="py-1.5 pr-2">
                <span
                  ...
                  style={{ background: f.sev === "high" ? WARM : "var(--ink-400)" }}
                  aria-label={f.sev === "high" ? "high severity" : "medium severity"}
                />
              </td>
              <td className="py-1.5 pr-3 font-mono" style={{ color: "var(--ink-900)" }}>
                {f.code}
              </td>
    ...
    <p className="mt-3 text-[11px] leading-relaxed" style={{ color: "var(--ink-500)" }}>
      ...
      <span className="font-mono">inspect</span> — and cites them in its message;
      the doctor never re-encodes, so every delta is labelled{" "}
  ...
  <Card>
    ...
    <div className="mt-3 overflow-x-auto">
      <pre
        ...
        style={{ background: "var(--surface-sunken)", border: "1px solid var(--hairline)", color: "var(--ink-700)", fontFamily: MONO }}
      >{`"style_hints": {
  "layer_kind": "trips",
