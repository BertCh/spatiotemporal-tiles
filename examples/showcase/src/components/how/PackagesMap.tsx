...
const Card: React.FC<{ title: string; children: React.ReactNode; foot?: React.ReactNode }> = ({
  title,
  children,
  foot,
}) => (
...
const RustPanel: React.FC = () => (
  ...
  >
    <svg viewBox="0 0 440 250" className="w-full min-w-[400px]" role="img" aria-label="Diagram: the spatiotemporal-tiles facade crate ships four CLI binaries and sits on the stt-build and stt-optimize libraries, which both sit on stt-core, the format implementation.">
      <defs>
        <marker id="arr-rs" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
          <path d="M 0 0 L 8 4 L 0 8 z" fill="var(--ink-400)" />
        </marker>
      </defs>

      <Box x={20} y={14} w={400} h={70} title="spatiotemporal-tiles" sub="crates.io · one install, four CLIs" accent />
      <text x="220" y="74" fontSize="9.5" fontFamily={MONO} textAnchor="middle" fill="var(--accent)">
        stt-build · stt-optimize · stt-validate · stt-serve
      </text>

      <line x1="110" y1="84" x2="110" y2="112" stroke="var(--ink-400)" strokeWidth="1" markerEnd="url(#arr-rs)" />
      <line x1="330" y1="84" x2="330" y2="112" stroke="var(--ink-400)" strokeWidth="1" markerEnd="url(#arr-rs)" />

      <Box x={40} y={116} w={140} h={42} title="stt-build" sub="the tiler / encoder" />
      <Box x={260} y={116} w={140} h={42} title="stt-optimize" sub="analyzer — powers --auto" />

      <line x1="110" y1="158" x2="110" y2="186" stroke="var(--ink-400)" strokeWidth="1" markerEnd="url(#arr-rs)" />
      <line x1="330" y1="158" x2="330" y2="186" stroke="var(--ink-400)" strokeWidth="1" markerEnd="url(#arr-rs)" />

      <Box x={40} y={190} w={360} h={48} title="stt-core" sub="the format — packs · directory · Arrow tiles · projection" />
    </svg>
  </Card>
);
...
const TsPanel: React.FC = () => (
  ...
  >
    <svg viewBox="0 0 440 262" className="w-full min-w-[400px]" role="img" aria-label="Diagram: five renderer and UI packages sit on @poopdeck.gl/core; layers, three and react additionally sit on the zero-dependency @poopdeck.gl/playback.">
      {TS_TOP.map((p) => (
      ...
      {TS_TOP.map((p) => (
        <Box key={p.name} x={p.x} y={16} w={76} h={48} title={`/${p.name}`} sub={p.sub} />
      ))}

      <Box x={10} y={170} w={230} h={52} title="@poopdeck.gl/core" sub="decoder · tileset · scheduler" accent />
      <Box x={262} y={170} w={168} h={52} title="…/playback" sub="clock · governor · zero deps" accent />

      {/* legend */}
      <line x1="14" y1="248" x2="34" y2="248" stroke="var(--ink-400)" strokeWidth="1.2" opacity="0.7" />
      <text x="40" y="251" fontSize="9.5" fill="var(--ink-500)">
        tiles (depends on core)
      </text>
      <line x1="170" y1="248" x2="190" y2="248" stroke="var(--accent)" strokeWidth="1.2" opacity="0.8" />
...
const MatrixCell: React.FC<{ cell: Cell }> = ({ cell }) => {
  ...
  if (cell === "-")
    return (
      <span aria-label="unsupported" style={{ color: "var(--ink-400)", opacity: 0.6 }}>
        —
      </span>
    );
  return (
    <span className="font-mono text-[9.5px]" style={{ color: "var(--ink-500)" }}>
      {cell}
    </span>
  );
};

const MatrixTable: React.FC<{ rows: MatrixRow[]; label: string }> = ({ rows, label }) => (
  <table className="border-collapse w-full text-[11px]">
    <thead>
      <tr>
        <th className="text-left font-medium pb-1.5 pr-2" style={{ color: "var(--ink-400)" }}>
          {label}
        </th>
        {BACKENDS.map((b) => (
          <th key={b} className="font-mono font-medium pb-1.5 px-2 text-center" style={{ color: "var(--ink-500)" }}>
            {b}
          </th>
        ))}
      </tr>
    ...
    <tbody>
      {rows.map((r) => (
        <tr key={r.kind} style={{ borderTop: "1px solid var(--hairline)" }}>
          <td className="py-1 pr-2 font-mono" style={{ color: "var(--ink-700)" }}>
            {r.kind}
          </td>
          {r.cells.map((c, i) => (
  ...
  <Card
    ...
    foot={
      <>
        ...
        <Link to="/docs/spec/backend-capabilities" style={{ color: "var(--accent)" }}>
          the spec
        </Link>
        .
      </>
  ...
  >
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-6">
      ...
      <div className="min-w-[300px] lg:w-72">
        ...
        <p className="mt-3 text-[10.5px] leading-relaxed" style={{ color: "var(--ink-400)" }}>
          deck.gl is the reference backend; Three (WebGPU/TSL) tracks it
          closest and adds the ego/cockpit kinds; MapLibre covers the
          basemap-native core; Cesium focuses on the movement catalog on a
          WGS84 globe.
        </p>
      </div>
    </div>
