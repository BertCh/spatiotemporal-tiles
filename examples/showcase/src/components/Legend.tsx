...
const Legend: React.FC<LegendProps> = ({ legend }) => {
  ...
  return (
    <div
      ...
      style={{ background: 'rgba(36, 39, 48, 0.95)', border: '1px solid #3A414C', minWidth: 160 }}
    >
      ...
      {hasRamps && (
        <div className="space-y-2 mb-2">
          {legend.ramps!.map((ramp, idx) => (
            <div key={idx}>
              <div className="text-[10px] mb-1" style={{ color: '#A0A7B4' }}>{ramp.label}</div>
              <div
              ...
              <div className="flex justify-between mt-0.5">
                <span className="text-[9px]" style={{ color: '#6A7485' }}>low</span>
                <span className="text-[9px]" style={{ color: '#6A7485' }}>high</span>
              </div>
            </div>
          ))}
        </div>
