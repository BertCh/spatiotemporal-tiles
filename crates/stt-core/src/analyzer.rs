...
impl ArchiveStats {
    ...
    pub fn print_report(&self) {
        ...
        println!("  Uncompressed:      {:.2} MB", self.uncompressed_size as f64 / 1_048_576.0);
        println!("  Compressed:        {:.2} MB", self.compressed_size as f64 / 1_048_576.0);
        println!("  Compression Ratio: {:.2}x", 1.0 / self.compression_ratio());
        println!();

        println!("🔍 Tile Size Distribution");
        println!("  Min:     {:.2} KB", self.size_distribution.min_size as f64 / 1024.0);
        println!("  Avg:     {:.2} KB", self.size_distribution.avg_size / 1024.0);
        println!("  Median:  {:.2} KB", self.size_distribution.median_size as f64 / 1024.0);
        println!("  P95:     {:.2} KB", self.size_distribution.p95_size as f64 / 1024.0);
        println!("  P99:     {:.2} KB", self.size_distribution.p99_size as f64 / 1024.0);
        println!("  Max:     {:.2} KB", self.size_distribution.max_size as f64 / 1024.0);
        println!();

        println!("📈 Feature Density");
        println!("  Min features/tile:  {}", self.density_stats.min_features);
        println!("  Avg features/tile:  {:.1}", self.density_stats.avg_features);
        println!("  Median:             {}", self.density_stats.median_features);
        println!("  Max features/tile:  {}", self.density_stats.max_features);
        println!("  Oversized tiles:    {}", self.density_stats.oversized_tiles);
        println!("  Undersized tiles:   {}", self.density_stats.undersized_tiles);
        println!();

        println!("⏰ Temporal Distribution");
        println!("  Unique timestamps:  {}", self.temporal_stats.unique_timestamps);
        println!("  Avg tiles/time:     {:.1}", self.temporal_stats.avg_tiles_per_timestamp);
        if self.temporal_stats.suggested_bucket_size > 0 {
            println!("  Suggested bucket:   {} ms", self.temporal_stats.suggested_bucket_size);
        }
        println!();
...
impl ArchiveAnalyzer {
    ...
    pub fn analyze(reader: &mut ArchiveReader) -> Result<ArchiveStats> {
        ...
        for entry in &index.tiles {
            ...
            // Update zoom stats
            let zoom_stat = stats.zoom_stats
                .entry(tile_id.z)
        ...
        if timestamps.len() > 0 {
            ...
            if time_span > 0 && timestamps.len() > 1000 {
                ...
                stats.temporal_stats.suggested_bucket_size = 
                    time_span / 1500; // Target ~1500 buckets
            }
        }
