#!/usr/bin/env bash
# Regenerate the conformance VECTORS under conformance/vectors/ with the REAL
# Rust writer (stt-build, which emits the only supported packed format, v2).
#
# These are the reader-side oracle: six small datasets that exercise the shapes
# a conformant reader must open. They lived at packages/core/test/fixtures/
# until the 2026-08-26 repository split, which made explicit what was already
# true — they are produced HERE, by the writer, and the TypeScript reader
# vendors them (docs/roadmap/repo-split-2026-08.md §4.2). Any third-party
# reader can do the same; conformance/README.md says how.
#
...
# dictionary columns still carrying a DictionaryBatch each) is not expressible
# as a vector here. The reader covers it with synthesized inline-schema frames
# instead, mirroring the Rust writer's empty-tile shape from
# crates/stt-core/tests/v2_golden.rs.
#
# Usage: conformance/make-vectors.sh
# Requires: cargo (builds spatiotemporal-tiles with `--features duckdb`).

set -euo pipefail

REPO="$(cd "$(dirname "$0")/.." && pwd)"
FIXTURES="$REPO/conformance/vectors"
