...
describe("PlaybackControls multi-source runway strip", () => {
  it("renders one track per source and highlights the gating source", () => {
    ...
    const runways: SourceRunway[] = [
      { id: "field", required: true, runwaySimMs: 4_000, complete: false, bytesPending: 10 },
      { id: "cells", required: true, runwaySimMs: 1_000, complete: false, bytesPending: 50 }, // gating
      { id: "tracks", required: false, runwaySimMs: 8_000, complete: false, bytesPending: 0 },
    ];
    ...
    ).filter((el) => /field|cells|tracks/.test(el.getAttribute("title") ?? ""));
  ...
  it("renders NO strip for a single source (degrades to the plain bar)", () => {
    const runways: SourceRunway[] = [
      { id: "default", required: true, runwaySimMs: 4_000, complete: false, bytesPending: 0 },
    ];
    const governor = makeMockGovernor(runways);
  ...
  it("highlights nothing when every required source is complete", () => {
    const runways: SourceRunway[] = [
      { id: "a", required: true, runwaySimMs: 9_000, complete: true, bytesPending: 0 },
      { id: "b", required: true, runwaySimMs: 9_000, complete: true, bytesPending: 0 },
    ];
    const governor = makeMockGovernor(runways);
    ...
    expect(
      container.querySelectorAll('[title*="gating"]').length,
    ).toBe(0);
