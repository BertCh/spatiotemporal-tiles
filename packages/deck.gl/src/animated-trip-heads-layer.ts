 ...
 *
 * This is the vanilla-stack replacement for {@link VatTripsLayer}'s head-dot
 * mode. The head position (where the vehicle sits at the current playhead) is
 ...
 * (no high-zoom jitter), real circular markers, globe support, and — unlike the
 * texture-baked VAT path — none of fp32 jitter / no-globe / constant-color
 * limitations.
 *
 ...
 * tiles — a few thousand at most at showcase scale (well under 1 ms/frame). VAT
 * moved this work to a per-tile texture sampled in the vertex shader; this
 * layer trades that for stock-layer features and zero custom GLSL.
 *
 ...
 * (bridge/highway) geometry — the dot glides exactly like the VAT head did.
 *
...
> {
   ...
   * is implicit, matching VAT's in-window vertex cull (a trip pops in at its
   * start and out at its end, no fade, exactly as the VAT head did).
   *
    ...
    for (const tile of tiles) {
      for (const tileLayer of tile.layers) {
        ...
        const props = this.composeSubLayerProps('heads', prepared.tileKey, {
          ...
          // pick would mis-map to a feature. Mirrors the VAT head (also
          // non-pickable); aligned picking (emit an instanceFeatureIndex) is a
          // follow-up.
          pickable: false,
          getFillColor: color,
          getRadius: radius,
