 ...
 * ```
 */
export class SpatioTemporalTileLayer extends TileLayer {
    initializeState() {
    ...
    async _initArchive() {
        ...
        if (!url)
            return;
        if (DEBUG)
            console.log('[STTLayer] Initializing archive:', url);
     ...
     */
    async _loadTileWithTime(x, y, z, timeStart, timeEnd, signal) {
    ...
    _filterTilesByTime() {
        ...
        const visibleTiles = tileset.tiles.filter((tile) => {
            if (!tile.content)
                return false;
...
SpatioTemporalTileLayer.layerName = 'SpatioTemporalTileLayer';
SpatioTemporalTileLayer.defaultProps = defaultProps;
export default SpatioTemporalTileLayer;
//# sourceMappingURL=spatiotemporal-tile-layer.js.map