...
export class SpatioTemporalLayer extends CompositeLayer {
    ...
    async initArchive() {
        if (DEBUG)
            console.log('SpatioTemporalLayer: Initializing archive from', this.props.data);
        const archive = new STTArchive(this.props.data);
        this.setState({ archive });
        // Load initial tiles
        await this.loadTilesForTime(this.props.currentTime);
    }
    async loadTilesForTime(time) {
        const { archive } = this.state;
        if (!archive) {
            if (DEBUG)
                console.log('SpatioTemporalLayer: No archive, skipping tile load');
            return;
        }
        // Get viewport bounds
        ...
        if (!viewport) {
            if (DEBUG)
                console.log('SpatioTemporalLayer: No viewport available');
            return;
        }
        ...
        try {
            ...
            if (DEBUG)
                console.log(`SpatioTemporalLayer: Initial load with ${initialTimeWindow / 86400000} day window`);
            const tiles = await archive.getTilesInBounds(bounds, zoom, initialTimeRange);
            if (DEBUG)
                console.log(`SpatioTemporalLayer: Loaded ${tiles.length} tiles`);
            ...
            if (tiles.length > 0 && tiles[0].temporalResolution) {
                ...
                if (DEBUG)
                    console.log('SpatioTemporalLayer: Auto-configuring from tile metadata:', {
                ...
                if (DEBUG)
                    console.log(`SpatioTemporalLayer: Reloaded with ${autoTimeWindow / 86400000}day window: ${tilesWithWindow.length} tiles`);
                ...
                if (this.props.timeController && tempRes.suggestedSpeedMultiplier > 1) {
                    ...
                    if (DEBUG)
                        console.log(`SpatioTemporalLayer: Recommending animation speed: ${recommendedSpeed / 86400000} days/sec`);
    ...
    async prefetchNearbyTiles(bounds, zoom, currentTime) {
        const { archive } = this.state;
        if (!archive)
            return;
...
//# sourceMappingURL=spatiotemporal-layer-old.js.map