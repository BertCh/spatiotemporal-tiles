...
import { buildViewMap, inferLayerType, resolveManifestUrl } from '../src/view-map';
import type { DatasetDescription } from '../src/manifest';

function dataset(overrides: Partial<DatasetDescription> = {}): DatasetDescription {
  return {
    name: 'earthquakes',
...
describe('inferLayerType', () => {
  it('prefers style_hints.layer_hint (confidence: hint)', () => {
    expect(inferLayerType(dataset({ styleHints: { version: 1, properties: [], layer_hint: 'trips' } }))).toEqual({
  ...
  it('falls back to the summary-tier scheme (confidence: summary)', () => {
    expect(inferLayerType(dataset({ hasSummaryTier: true, summaryScheme: 'h3' }))).toEqual({
      type: 'H3SummaryLayer',
      confidence: 'summary',
    });
    expect(inferLayerType(dataset({ hasSummaryTier: true, summaryScheme: 'quadbin' }))).toEqual({
  ...
  it('blindly defaults to AnimatedPointLayer (confidence: default)', () => {
    expect(inferLayerType(dataset())).toEqual({ type: 'AnimatedPointLayer', confidence: 'default' });
  });
});

describe('resolveManifestUrl', () => {
  it('uses the filesystem path when no publicBaseUrl is configured', () => {
    expect(resolveManifestUrl(dataset())).toBe('/data/earthquakes/manifest.json');
  });
...
describe('buildViewMap', () => {
  it('builds one layer per dataset with the inferred @@type and manifest URL', () => {
    const { spec } = buildViewMap([dataset()], { publicBaseUrl: 'https://tiles.example.com' });
    ...
    expect(layer.data).toBe('https://tiles.example.com/earthquakes/manifest.json');
  ...
  it('honors an explicit layer override for every dataset', () => {
    const { spec } = buildViewMap([dataset(), dataset({ name: 'other', path: '/data/other' })], {
  ...
  it('honors an explicit time and viewState', () => {
    ...
    expect(spec.initialViewState).toMatchObject({ longitude: 1, latitude: 2, zoom: 9 });
  });
  ...
  it('zooms to the union extent (not the coarsest minZoom) across datasets', () => {
    ...
    const { spec } = buildViewMap(
      [
        dataset({ boundingBox: { minLon: -10, minLat: -10, maxLon: 0, maxLat: 0 }, minZoom: 4 }),
        dataset({ boundingBox: { minLon: 0, minLat: 0, maxLon: 10, maxLat: 10 }, minZoom: 2 }),
      ],
      { publicBaseUrl: 'https://tiles.example.com' },
    );
    expect(spec.initialViewState.longitude).toBeCloseTo(0);
  ...
  it('zooms out sensibly when disjoint datasets union to a wide bbox', () => {
    const { spec } = buildViewMap(
      [
        dataset({ boundingBox: { minLon: -170, minLat: -10, maxLon: -160, maxLat: 0 } }),
        dataset({ boundingBox: { minLon: 160, minLat: 0, maxLon: 170, maxLat: 10 } }),
      ],
      { publicBaseUrl: 'https://tiles.example.com' },
    );
  ...
  it('clamps the extent zoom to the data maxZoom for a degenerate (point) bbox', () => {
    const { spec } = buildViewMap([dataset({ boundingBox: { minLon: 5, minLat: 5, maxLon: 5, maxLat: 5 }, maxZoom: 9 })], {
      publicBaseUrl: 'https://tiles.example.com',
    });
  ...
  it('warns when the geometry type is unknown (blind default)', () => {
    const { warnings } = buildViewMap([dataset()], { publicBaseUrl: 'https://tiles.example.com' });
    expect(warnings.some((w) => w.includes('geometry type is unknown') && w.includes('earthquakes'))).toBe(true);
  ...
  it('does NOT warn about geometry when a hint gives a positive signal', () => {
    const { warnings } = buildViewMap(
      [dataset({ styleHints: { version: 1, properties: [], layer_hint: 'paths' } })],
      { publicBaseUrl: 'https://tiles.example.com' },
    );
    expect(warnings.some((w) => w.includes('geometry type is unknown'))).toBe(false);
  });
  ...
  it('warns once that filesystem-path manifest URLs are not browser-fetchable', () => {
    const { warnings } = buildViewMap([dataset(), dataset({ name: 'other', path: '/data/other' })]);
    const fsWarnings = warnings.filter((w) => w.includes('local filesystem paths'));
  ...
  it('does NOT warn about filesystem paths when a publicBaseUrl is set', () => {
    const { warnings } = buildViewMap([dataset()], { publicBaseUrl: 'https://tiles.example.com' });
    expect(warnings.some((w) => w.includes('local filesystem paths'))).toBe(false);
  });

  it('produces a self-contained spec-preview HTML page (not a live map) embedding the spec JSON', () => {
    const { spec, html } = buildViewMap([dataset()], { publicBaseUrl: 'https://tiles.example.com' });
