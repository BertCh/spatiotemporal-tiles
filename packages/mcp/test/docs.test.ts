...
import { writeFixtureDocsRoot, writeFixtureDataRoot, cleanupDataRoot } from './fixtures';
...
function baseConfig(dataRoot: string, docsRoot: string): SttMcpConfig {
  return { dataRoot, docsRoot, allowCli: false, transport: 'stdio', host: '127.0.0.1', port: 0 };
}
...
describe('docs resource — stt://docs/<path>', () => {
  it('lists the published corpus and excludes roadmap/', async () => {
    const { client, close } = await connectedClient(baseConfig(await emptyDataRoot(), await docsFixture()));
    try {
      ...
      const docNames = resources
        ...
        .map((r) => r.name)
        .sort();
      expect(docNames).toEqual(['README.md', 'api/cli-reference.md', 'guides/ai-suite.md']);
  ...
  it('reads one doc as markdown', async () => {
    const { client, close } = await connectedClient(baseConfig(await emptyDataRoot(), await docsFixture()));
    try {
      const read = await client.readResource({ uri: 'stt://docs/api%2Fcli-reference.md' });
...
describe('get_doc tool', () => {
  it('returns the markdown content of a corpus doc', async () => {
    const { client, close } = await connectedClient(baseConfig(await emptyDataRoot(), await docsFixture()));
    try {
      const result = await client.callTool({ name: 'get_doc', arguments: { path: 'api/cli-reference.md' } });
  ...
  it('rejects an absolute path', async () => {
    const { client, close } = await connectedClient(baseConfig(await emptyDataRoot(), await docsFixture()));
    try {
      const result = await client.callTool({ name: 'get_doc', arguments: { path: '/etc/passwd' } });
      expect(result.isError).toBe(true);
    } finally {
      await close();
    }
  ...
  it('truncates oversized content with a marker reporting the true length', async () => {
    ...
    roots.push(docsRoot);
    const { client, close } = await connectedClient(baseConfig(await emptyDataRoot(), docsRoot));
    try {
      const result = await client.callTool({
        name: 'get_doc',
...
describe('search_docs tool', () => {
  it('finds a known term and returns ranked snippets with line numbers', async () => {
    const { client, close } = await connectedClient(baseConfig(await emptyDataRoot(), await docsFixture()));
    try {
      const result = await client.callTool({ name: 'search_docs', arguments: { query: 'stt-build' } });
      const parsed = JSON.parse(firstText(result));
      expect(parsed.count).toBeGreaterThanOrEqual(1);
      const hit = parsed.results.find((r: any) => r.path === 'api/cli-reference.md');
  ...
  it('never matches inside a non-corpus doc', async () => {
    const { client, close } = await connectedClient(baseConfig(await emptyDataRoot(), await docsFixture()));
    try {
      const result = await client.callTool({ name: 'search_docs', arguments: { query: 'Secret Plan' } });
      const parsed = JSON.parse(firstText(result));
...
describe('resolveDocPath (unit)', () => {
  it('accepts a corpus path and rejects traversal, absolutes, and non-corpus paths', () => {
    const root = '/docs';
    expect(resolveDocPath(root, 'api/cli-reference.md')).toBe('/docs/api/cli-reference.md');
    expect(resolveDocPath(root, 'README.md')).toBe('/docs/README.md');
    expect(() => resolveDocPath(root, '../../etc/passwd')).toThrow(/outside the docs root/);
    expect(() => resolveDocPath(root, '/etc/passwd')).toThrow(/absolute/);
    expect(() => resolveDocPath(root, 'roadmap/secret-plan.md')).toThrow(/not a published doc/);
    expect(() => resolveDocPath(root, 'api/nested/deep.md')).toThrow(/not a published doc/);
    expect(() => resolveDocPath(root, 'api/notes.txt')).toThrow(/not a published doc/);
  });
});
