 ...
 * `thicknessUnit`), this layer takes **width directly in pixels** per instance
 * (`getWidth`) — the {@link FlowmapLayer} already computes `widthScale·√flow`
 * in pixels, so we keep that pipeline and only swap the geometry from arc to
 * arrow. Color is a layer-uniform `mix(sourceColor → targetColor)` along the
 * arrow (our colors are constants, so no per-instance color attribute).
 *
 * PICKING / SHADER HOOKS: this is a fully custom-`Model` layer, so it calls the
 * `picking` module functions DIRECTLY (`picking_setPickingColor` in the VS,
 ...
 * affect this layer's shaders — {@link FlowmapLayer} doesn't need them (its
 * animation is CPU matrix-width and its colors are constants).
 */
...
in float instanceWidth;       // pixels
in vec2 instanceEndpointOffsets; // pixels: [source inset, target inset]
...
void main(void) {
  ...
  // Screen-space direction + length (pixels). Clip space spans [-1,1] across
  // the viewport, so clip→pixels is clip.xy * viewportSize / 2.
  vec2 deltaScreen = (target.xy - source.xy) * project.viewportSize / 2.0;
  ...
  float w = clamp(instanceWidth, flowLines.widthMinPixels, flowLines.widthMaxPixels);
  ...
  // Picking: propagate the per-instance picking color (and depth) ourselves —
  // see the file docstring on why we don't use deck's DECKGL_FILTER_* hooks.
...
type _FlowLinesLayerProps<DataT> = {
  ...
  /** Per-flow width in PIXELS (already scaled; not normalized). @default 1 */
  getWidth?: Accessor<DataT, number>;
  /**
   * Per-flow `[sourceInset, targetInset]` in pixels — how far the arrow's
  ...
  targetColor?: Color;
  /** Clamp width to at least this many pixels. @default 0 */
  widthMinPixels?: number;
