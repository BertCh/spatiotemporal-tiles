...
describe('AnimatedPointCloudLayer per-tile sublayer architecture', () => {
  ...
  it('builds one PointCloudLayer per tile (no cross-tile consolidation)', () => {
    const layer = makeLayer();
  ...
  it('returns the SAME PointCloudLayer instance per tile across renders when nothing changed', () => {
    const layer = makeLayer();
    const a = bigPointTile(3);
    a.id = { z: 14, x: 1, y: 2, t: 0 };
    const b = bigPointTile(3);
  ...
  it('rebuilds the cached PointCloudLayer when a layer-level prop changes', () => {
    const layer = makeLayer();
    const tile = bigPointTile(3);
    layer.state = { tiles: [tile] };
    const first = (layer as any).renderLayers();
    layer.props.pointSize = 25;
    const second = (layer as any).renderLayers();
    expect(second[0]).not.toBe(first[0]);
    expect(second[0].props.pointSize).toBe(25);
  });
