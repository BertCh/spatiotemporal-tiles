 ...
 */
export function getBinaryPosition3D(binary, featureIndex) {
    const dims = binary.positionDimensions ?? 2;
    const offset = featureIndex * dims;
    return [
 ...
 */
export function getCategoricalProperty(binary, propertyName, featureIndex) {
 ...
 */
export function getCategoricalIndex(binary, propertyName, featureIndex) {
    const prop = binary.categoricalProps[propertyName];
 ...
 */
export function createEmptyBinaryFeatures(geometryType = 0) {
    return {
        featureCount: 0,
        geometryType,
 ...
 */
export function isFeatureVisible(binary, featureIndex, currentTime, timeWindow) {
 ...
 */
export function getFeaturePositionCount(binary, featureIndex) {
    if (!binary.startIndices) {
        // Point geometry
        return 1;
    }
