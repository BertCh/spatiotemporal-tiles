...
function collectLineLayers(tiles: Tile[]): { layers: BinaryFeatures[]; segCount: number } {
  ...
  for (const b of layers) {
    ...
    for (let f = 0; f < b.featureCount; f++) {
      ...
      for (let v = v0; v < v1 - 1; v++) {
        const z0 = (baseZ ?? (dims > 2 ? b.positions[v * dims + 2] : 0)) + zLift;
        const z1 = (baseZ ?? (dims > 2 ? b.positions[(v + 1) * dims + 2] : 0)) + zLift;
