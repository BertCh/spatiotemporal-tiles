...
const PACKAGES = ["core", "playback", "layers", "maplibre", "three", "cesium", "react"];
...
function packAll(dest) {
  ...
  for (const p of PACKAGES) {
    sh(`pnpm pack --pack-destination ${JSON.stringify(dest)}`, join(ROOT, "packages", p));
  }
...
function scratchProject(dir, installs) {
  ...
  sh(`npm install --no-audit --no-fund --loglevel=error ${installs.map((i) => JSON.stringify(i)).join(" ")}`, dir);
}
...
const work = mkdtempSync(join(process.env.RUNNER_TEMP ?? tmpdir(), "stt-smoke-"));
...
const fullFailures = importAllExports(full, PACKAGES.map((p) => `@poopdeck.gl/${p}`));
...
const bTarballs = tarballs.filter((t) => /(core|playback|react)-0\./.test(t) && !/three/.test(t));
scratchProject(deckFree, [...bTarballs, "react@19.2.0", "react-dom@19.2.0"]);
let deckFreeFailed = false;
try {
  sh(`node --input-type=module -e "await import('@poopdeck.gl/react')"`, deckFree);
  console.log("  ok   @poopdeck.gl/react (no deck.gl installed)");
} catch (e) {
  deckFreeFailed = true;
  console.error(`  FAIL @poopdeck.gl/react without deck.gl — ${String(e.stderr).split("\n")[0]}`);
}
